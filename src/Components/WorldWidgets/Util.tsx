...
import { Button, Menu, MenuItem, Alert, AlertTitle } from '@mui/material';
...
export function StdOutAndStdErr(props){
  ...
  return(
    ...
    {stdOut.length === 0 ?
      <></> :
      <Alert severity="warning" style={{ overflowX: "auto" }}>{stdOut.map((e:any, i) => {
        return <div key={i}>
      ...
      {stdErr.length === 0 ?
      <></> :
      <Alert severity="error" style={{ overflowX: "auto" }}>{stdErr.map((e:any, i) => {
...
interface PastEvents{
  "projectile-hit": Array<any>,
  "zone-enter": Array<any>
}
...
subscribeToUnrealEvent("projectile-hit",(data:any)=>{
  pastEvents["projectile-hit"].unshift(data)
})

subscribeToUnrealEvent("zone-enter",(data:any)=>{
